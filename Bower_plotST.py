...
parser.add_argument('-l','--stmin', default=1300., help='Min ST to plot.',type=float)
parser.add_argument('-r','--stmax', default=3500., help='Max ST to plot.',type=float)
parser.add_argument('-b','--nbins', default=6, help='Number of bins over which to plot ST.',type=int)
...
parser.add_argument('-t','--type' , default='', help='Ntuple Type(Data/MC)',type=str)
...
nPho  = 1
evtHT = args.HTcut 
edges = [1300., 1500., 1900., 2300., 2700.,3100.,3500.]
if args.HTcut < 100.:
    nJtMax = 6 
    #nJtMax = 3 
    #nPho = 2 
...
def main():
    ...
    for j in range(nJtMin,nJtMax+1):
        hST.append( ROOT.TH1F("h" + str(j) + "jet", str(j)+"jet", 5 , array("d",edges)) )
        hST[-1].Sumw2()
    ...
    c1.Print("binned_1fake1real_sT_2016%s_%s.png" %(args.era, args.type))
#    c1.Print("FakeReal/ST_Plots/binned_1fake1real_sT_2016%s_%s.eps" %(args.era, args.type))
